...
function ClozeTestBuilder() {
  ...
  return (
    <div className="p-4 h-fit w-4/6 shadow-lg rounded-lg mb-10 border-2 border-emerald-800" style={{ backgroundColor: 'lightgray' }}>
      <p className="font-semibold">Cloze Test</p>
      <div className="mt-4 mb-3">
        <label className="mb-2 mr-2 inline font-semibold">Preview:</label>
        <p className="border-b border-rose-800 shadow-sm rounded p-1 w-fit w-min-[50%] inline font-semibold">
          {underlineSelectedWords()}
        </p>
      </div>
      <label htmlFor="sentenceInput" className="block mb-2 font-semibold text-sm text-red-700">
        Type your sentence and double click on the words to make it an option:
      </label>
      ...
      <div className="mt-4">
        ...
        <ul className="flex justify-evenly w-fit">
          {selectedWords.map((word, index) => (
            <li className="flex justify-center font-semibold rounded-md p-1 m-2 bg-blue-900 text-white" key={index}>
              {word}{' '}
